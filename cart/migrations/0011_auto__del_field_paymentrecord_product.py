...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'PayMentRecord.product'
        db.delete_column(u'cart_paymentrecord', 'product_id')

        # Adding M2M table for field product on 'PayMentRecord'
        m2m_table_name = db.shorten_name(u'cart_paymentrecord_product')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('paymentrecord', models.ForeignKey(orm[u'cart.paymentrecord'], null=False)),
            ('productinfo', models.ForeignKey(orm[u'cart.productinfo'], null=False))
        ))
        db.create_unique(m2m_table_name, ['paymentrecord_id', 'productinfo_id'])


    def backwards(self, orm):
        # Adding field 'PayMentRecord.product'
        db.add_column(u'cart_paymentrecord', 'product',
                      self.gf('django.db.models.fields.related.ForeignKey')(default='', related_name='record_product', to=orm['cart.ProductInfo']),
                      keep_default=False)

        # Removing M2M table for field product on 'PayMentRecord'
        db.delete_table(db.shorten_name(u'cart_paymentrecord_product'))
